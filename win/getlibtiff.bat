...
call %~dp0configvars
set key=libtiff
if not exist %key%-source git clone https://gitlab.com/libtiff/libtiff.git %_buildroot_%/%key%-source
...
cd build
call %__%\trysetvcenv
msbuild ALL_BUILD.vcxproj /p:configuration=Release /p:OutDir=%key%-build/
